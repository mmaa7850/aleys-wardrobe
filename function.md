...
| 購物車 | `/cart` | 品項列表（**勾選欄**（僅非預購品項）、圖、名稱、規格、**預購預計出貨日**、數量 ± 控制、**刪除**（**僅預購品項顯示刪除按鈕**，現貨/直播入單/小編手動入單不顯示））；預購品項不顯示勾選框、不計入合計；**已選取合計金額**；前往結帳按鈕（無勾選時 disabled）；`cart.initSelection()` 在載入後自動勾選所有非預購品項 |
