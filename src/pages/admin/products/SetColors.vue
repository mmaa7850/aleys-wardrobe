...
const titleText = computed(() =>
  mode.value === "create" ? t("product.setcolors.createTitle") : t("product.setcolors.editTitle")
);
...
const validateForm = () => {
  ...
  if (!name) return t("product.setcolors.validateNameRequired");
  if (!desc) return t("product.setcolors.validateDescriptionRequired");
...
const updateColor = async () => {
  ...
  if (!form.value.ID) {
    saveError.value = t("product.setcolors.missingId");
    return;
  }
...
const deleteRow = async (row) => {
  const ok = window.confirm(t("product.setcolors.confirmDelete", { name: row.Name }));
  if (!ok) return;
...
<template>
  <div class="container-fluid py-3">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
      <div>
        <h4 class="mb-1">{{ t("product.setcolors.title") }}</h4>
        <div class="text-muted small">{{ t("product.setcolors.tableName") }}</div>
      </div>
        ...
        <div v-else class="table-responsive">
          <table class="table table-hover mb-0 align-middle">
            <thead class="table-light">
              <tr>
                <th style="width: 30%">{{ t("product.setcolors.name") }}</th>
                <th style="width: 45%">{{ t("product.setcolors.description") }}</th>
                <th style="width: 20%">{{ t("product.setcolors.updatedDate") }}</th>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              <label class="form-label">{{ t("product.setcolors.name") }}</label>
              <input
                v-model="form.Name"
                type="text"
                class="form-control"
              ...
              />
              <div class="form-text">
                {{ mode === "edit" ? t("product.setcolors.nameHintEdit") : t("product.setcolors.nameHintCreate") }}
              </div>
            </div>

            <div class="mb-1">
              <label class="form-label">{{ t("product.setcolors.description") }}</label>
              <input
