...
const tableName = "S_SYS_ConfigCategoryList";
...
const titleText = computed(() =>
  mode.value === "create" ? t("settings.setconfigcategory.createTitle") : t("settings.setconfigcategory.editTitle")
);
...
const validateForm = () => {
  ...
  if (!name) return t("settings.setconfigcategory.validateNameRequired");
  if (!desc) return t("settings.setconfigcategory.validateDescriptionRequired");

  return "";
};

const createConfigCategorie = async () => {
  saveError.value = "";
...
const updateConfigCategorie = async () => {
  saveError.value = "";

  if (!form.value.ID) {
    saveError.value = t("settings.setconfigcategory.missingId");
    return;
  }
...
const submitModal = async () => {
  if (mode.value === "create") return createConfigCategorie();
  return updateConfigCategorie();
};

const deleteRow = async (row) => {
  const ok = window.confirm(t("settings.setconfigcategory.confirmDelete", { name: row.Name }));
  if (!ok) return;
...
<template>
  <div class="container-fluid py-3">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
      <div>
        <h4 class="mb-1">{{ t("settings.setconfigcategory.title") }}</h4>
        <div class="text-muted small">{{ t("settings.setconfigcategory.tableName") }}</div>
      </div>
        ...
        <div v-else class="table-responsive">
          <table class="table table-hover mb-0 align-middle">
            <thead class="table-light">
              <tr>
                <th style="width: 30%">{{ t("settings.setconfigcategory.name") }}</th>
                <th style="width: 45%">{{ t("settings.setconfigcategory.description") }}</th>
                <th style="width: 20%">{{ t("settings.setconfigcategory.updatedDate") }}</th>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              <label class="form-label">{{ t("settings.setconfigcategory.name") }}</label>
              <input
                v-model="form.Name"
                type="text"
                class="form-control"
              ...
              />
              <div class="form-text">
                {{ mode === "edit" ? t("settings.setconfigcategory.nameHintEdit") : t("settings.setconfigcategory.nameHintCreate") }}
              </div>
            </div>

            <div class="mb-1">
              <label class="form-label">{{ t("settings.setconfigcategory.description") }}</label>
              <input
