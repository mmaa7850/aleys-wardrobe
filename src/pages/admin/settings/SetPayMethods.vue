...
const tableName = "S_PAY_PayMethodList";
...
const titleText = computed(() =>
  mode.value === "create" ? t("settings.setpaymethod.createTitle") : t("settings.setpaymethod.editTitle")
);
...
const validateForm = () => {
  ...
  if (!name) return t("settings.setpaymethod.validateNameRequired");
  if (!desc) return t("settings.setpaymethod.validateDescriptionRequired");
...
const updatePayMethod = async () => {
  ...
  if (!form.value.ID) {
    saveError.value = t("settings.setpaymethod.missingId");
    return;
  }
...
const submitModal = async () => {
  if (mode.value === "create") return createPayMethod();
  return updatePayMethod();
};

const deleteRow = async (row) => {
  const ok = window.confirm(t("settings.setpaymethod.confirmDelete", { name: row.Name }));
  if (!ok) return;
...
<template>
  <div class="container-fluid py-3">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
      <div>
        <h4 class="mb-1">{{ t("settings.setpaymethod.title") }}</h4>
        <div class="text-muted small">{{ t("settings.setpaymethod.tableName") }}</div>
      </div>
        ...
        <div v-else class="table-responsive">
          <table class="table table-hover mb-0 align-middle">
            <thead class="table-light">
              <tr>
                <th style="width: 30%">{{ t("settings.setpaymethod.name") }}</th>
                <th style="width: 45%">{{ t("settings.setpaymethod.description") }}</th>
                <th style="width: 20%">{{ t("settings.setpaymethod.updatedDate") }}</th>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              <label class="form-label">{{ t("settings.setpaymethod.name") }}</label>
              <input
                v-model="form.Name"
                type="text"
                class="form-control"
              ...
              />
              <div class="form-text">
                {{ mode === "edit" ? t("settings.setpaymethod.nameHintEdit") : t("settings.setpaymethod.nameHintCreate") }}
              </div>
            </div>

            <div class="mb-1">
              <label class="form-label">{{ t("settings.setpaymethod.description") }}</label>
              <input
