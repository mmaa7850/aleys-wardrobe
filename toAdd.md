...
**問題背景：**
- 官網與蝦皮同時經營，庫存分開放（`StockQty` vs `ShopeeStockQty`）
- 目前 FIFO 批次不區分通路，進 80 件但蝦皮賣 40 件的成本不會出現在官網報表
- 蝦皮有耗材（包材/贈品）、廣告費等成本需要記錄，但蝦皮後台無法記錄

**待解決的三件事：**

1. **進貨成本通路分配**：進貨單確認時填「官網 X 件 / 蝦皮 Y 件」，FIFO 批次建兩筆分開的 pool，各通路成本各自計算
2. **蝦皮耗材記錄**：出蝦皮貨時記錄包材/贈品用量（蝦皮訂單手動輸入，或月結摘要）
3. **廣告費 + 平台費用**：費用記錄加「平台」欄位（官網 / 蝦皮），淨利報表可分通路顯示
